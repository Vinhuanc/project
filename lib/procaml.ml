...
let caseproof  varnm rules lhs rhs (variantnm, _variantvars) =
...
let inductionproof varnm _types  rules lhs rhs =
   ...
   List.(concat (map (caseproof  varnm  rules lhs rhs) variants)) @
   ...
   match declarations with
      ...
      | ProofDeclaration (nm, vars, Equality (lhs,rhs), Some (Induction varnm)) :: rest
         -> (* we got an induction hint so we do that for the statement *)
            inductionproof varnm types  rules lhs rhs :: prover types ((List.map typedVariableVariable vars,nm,lhs,rhs)::rules) rest
